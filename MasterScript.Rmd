...
Mod_FP_T2 <- function(t, y, p){
  r <- p[1]; B <- p[2]; h <- p[3]; e <- p[4]; delta <- p[5]; b <- p[6]
  P <- y[1]; I <- y[2]
  dP = r*P - I*(B*P/(1 + h*B*P))
...
post <- mapply(c, post.1, post.5, SIMPLIFY = FALSE)
n.sim <- 2000
...
Mod_FP_T2 <- function(t, y, p){
  r <- p[1]; B <- p[2]; h <- p[3]; b <- p[4]; e <- p[5]; delta <- p[6]
  P <- y[1]; I <- y[2]
  dP = r*P - I*(B*P/(1 + h*B*P))
  dI = b + e*(B*P/(1 + h*B*P))*I-delta*I
  list(c(dP, dI))
...
plot(1, type = "n", main = "Mod. Fenton and Perkins (2010), Type II FR (Damping Oscillations) | Parameter Values Drawn from Posterior Distributions (Global Model)", xlab = "Time", ylab = "Abundance", xlim = c(0, 200), ylim = c(0, 500))
for (i in 1:n.sim){
  lines(x = DampOsc_SA_Outputs[[i]]$Time, y = DampOsc_SA_Outputs[[i]]$P, col = adjustcolor("forestgreen", alpha = 0.2))
  lines(x = DampOsc_SA_Outputs[[i]]$Time, y = DampOsc_SA_Outputs[[i]]$I, col = adjustcolor("cornflowerblue", alpha = 0.2))
}

# Checking to see if all time series are same length
TSLengths <- c()
for (i in 1:n.sim){
  TSLengths[i] <- nrow(DampOsc_SA_Outputs[[i]])
}; length(unique(TSLengths))

# Classifying behaviour
Is.DampOsc <- c()
for (i in 1:n.sim){
  TS <- DampOsc_SA_Outputs[[i]][ , 2:3]
  Max.First10Days <- rev(sort(TS[1:100, 1]))[1:10]; Min.First10Days <- sort(TS[1:100, 1])[1:10]
  Max.Last10Days <- rev(sort(TS[(nrow(TS)-100):nrow(TS), 1]))[1:10]; Min.Last10Days <- sort(TS[(nrow(TS)-100):nrow(TS), 1])[1:10]
...
length(which(Is.DampOsc == "Y"))/length(Is.DampOsc) # 99.80%
```

### Mod. Fenton and Perkins, Type II FR, Stable Limit Cycle

```{r}
post.1 <- rstan::extract(ModFentonPerkins_2_SLC_FitObj1, pars = c("theta[1]", "theta[2]", "theta[3]", "theta[4]", "theta[5]", "theta[6]"))
post.2 <- rstan::extract(ModFentonPerkins_2_SLC_FitObj2, pars = c("theta[1]", "theta[2]", "theta[3]", "theta[4]", "theta[5]", "theta[6]"))
post.5 <- rstan::extract(ModFentonPerkins_2_SLC_FitObj5, pars = c("theta[1]", "theta[2]", "theta[3]", "theta[4]", "theta[5]", "theta[6]"))
post <- mapply(c, post.1, post.2, post.5, SIMPLIFY = FALSE)
n.sim <- 2000
set.seed(123); samps <- sample(length(post[[1]]), n.sim, replace = FALSE)
samp.post <- vector(mode = "list", length = length(post))
for (i in 1:n.sim){
...
SLC_SA_Outputs <- list(); SLC_TS_Length <- c()
for (i in 1:n.sim){
  r <- samp.post[[1]][i]; B <- samp.post[[2]][i]; h <- samp.post[[3]][i]
  b <- samp.post[[4]][i]; e <- samp.post[[5]][i]; delta <- samp.post[[6]][i]
  parms <- c(r, B, h, b, e, delta)
  ...
  results <- as.data.frame(lsoda(N0, TT, Mod_FP_T2, parms)); colnames(results) <- c("Time", "P", "I")
  # results <- results[1:which(results$P < 0)[1] - 1, ]
  SLC_SA_Outputs[[i]] <- results; SLC_TS_Length[i] <- nrow(results)/10
}

for (i in 1:n.sim){
  SLC_SA_Outputs[[i]] <- na.omit(SLC_SA_Outputs[[i]])
}

for (i in 1:n.sim){
  SLC_SA_Outputs[[i]] <- subset(SLC_SA_Outputs[[i]], P >= 0)
  SLC_SA_Outputs[[i]] <- subset(SLC_SA_Outputs[[i]], I >= 0)
}

plot(1, type = "n", main = "Mod. Fenton and Perkins (2010), Type II FR (Stable Limit Cycle) | Parameter Values Drawn from Posterior Distributions (Global Model)", xlab = "Time", ylab = "Abundance", xlim = c(0, 120), ylim = c(0, 1500))
for (i in 1:n.sim){
  lines(x = SLC_SA_Outputs[[i]]$Time, y = SLC_SA_Outputs[[i]]$P, col = adjustcolor("forestgreen", alpha = 0.2))
  lines(x = SLC_SA_Outputs[[i]]$Time, y = SLC_SA_Outputs[[i]]$I, col = adjustcolor("cornflowerblue", alpha = 0.2))
}

# Checking to see if all time series are same length
TSLengths <- c()
for (i in 1:n.sim){
  TSLengths[i] <- nrow(SLC_SA_Outputs[[i]])
}; unique(TSLengths); length(unique(TSLengths))

# Classifying behaviour
Is.SLC <- c()
for (i in 1:n.sim){
  TS <- SLC_SA_Outputs[[i]][ , 2:3]
  Max.First10Days <- rev(sort(TS[1:100, 1]))[1:10]; Min.First10Days <- sort(TS[1:100, 1])[1:10]
  Max.Last10Days <- rev(sort(TS[(nrow(TS)-100):nrow(TS), 1]))[1:10]; Min.Last10Days <- sort(TS[(nrow(TS)-100):nrow(TS), 1])[1:10]
  Diff.Start <- mean(Max.First10Days) - mean(Min.First10Days); Diff.End <- mean(Max.Last10Days) - mean(Min.Last10Days)
...
length(which(Is.SLC == "Y"))/length(Is.SLC) # 40.15%
```

### Mod. Fenton and Perkins, Type II FR, Divergent Oscillations

```{r}
post.2 <- rstan::extract(ModFentonPerkins_2_DivOsc_FitObj2, pars = c("theta[1]", "theta[2]", "theta[3]", "theta[4]", "theta[5]", "theta[6]"))
post.5 <- rstan::extract(ModFentonPerkins_2_DivOsc_FitObj5, pars = c("theta[1]", "theta[2]", "theta[3]", "theta[4]", "theta[5]", "theta[6]"))
post <- mapply(c, post.2, post.5, SIMPLIFY = FALSE)
n.sim <- 2000
set.seed(123); samps <- sample(length(post[[1]]), n.sim, replace = FALSE)
samp.post <- vector(mode = "list", length = length(post))
for (i in 1:n.sim){
...
DivOsc_SA_Outputs <- list(); DivOsc_TS_Length <- c()
for (i in 1:n.sim){
  r <- samp.post[[1]][i]; B <- samp.post[[2]][i]; h <- samp.post[[3]][i]
  b <- samp.post[[4]][i]; e <- samp.post[[5]][i]; delta <- samp.post[[6]][i]
  parms <- c(r, B, h, b, e, delta)
  ...
  results <- as.data.frame(lsoda(N0, TT, Mod_FP_T2, parms)); colnames(results) <- c("Time", "P", "I")
  # results <- results[1:which(results$P < 0)[1] - 1, ]
  DivOsc_SA_Outputs[[i]] <- results; DivOsc_TS_Length[i] <- nrow(results)/10
}

for (i in 1:n.sim){
  DivOsc_SA_Outputs[[i]] <- na.omit(DivOsc_SA_Outputs[[i]])
}

for (i in 1:n.sim){
  DivOsc_SA_Outputs[[i]] <- subset(DivOsc_SA_Outputs[[i]], P >= 0)
  DivOsc_SA_Outputs[[i]] <- subset(DivOsc_SA_Outputs[[i]], I >= 0)
}

plot(1, type = "n", main = "Mod. Fenton and Perkins (2010), Type II FR (Divergent Oscillations) | Parameter Values Drawn from Posterior Distributions (Global Model)", xlab = "Time", ylab = "Abundance", xlim = c(0, 120), ylim = c(0, 4000))
for (i in 1:n.sim){
  lines(x = DivOsc_SA_Outputs[[i]]$Time, y = DivOsc_SA_Outputs[[i]]$P, col = adjustcolor("forestgreen", alpha = 0.2))
  lines(x = DivOsc_SA_Outputs[[i]]$Time, y = DivOsc_SA_Outputs[[i]]$I, col = adjustcolor("cornflowerblue", alpha = 0.2))
}

# Checking to see if all time series are same length
TSLengths <- c()
for (i in 1:n.sim){
  TSLengths[i] <- nrow(DivOsc_SA_Outputs[[i]])
}; unique(TSLengths); length(unique(TSLengths))

# Classifying behaviour
Is.DivOsc <- c()
for (i in 1:n.sim){
  TS <- DivOsc_SA_Outputs[[i]][ , 2:3]
  Max.First10Days <- rev(sort(TS[1:100, 1]))[1:10]; Min.First10Days <- sort(TS[1:100, 1])[1:10]
  Max.Last10Days <- rev(sort(TS[(nrow(TS)-100):nrow(TS), 1]))[1:10]; Min.Last10Days <- sort(TS[(nrow(TS)-100):nrow(TS), 1])[1:10]
  Diff.Start <- mean(Max.First10Days) - mean(Min.First10Days); Diff.End <- mean(Max.Last10Days) - mean(Min.Last10Days)
  if (Diff.Start < Diff.End){
...
length(which(Is.DivOsc == "Y"))/length(Is.DivOsc) # 90.20%
```
