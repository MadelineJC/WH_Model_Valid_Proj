...
fit_Overlay <- mcmc_dens_overlay(Fit_Antia, parms); fit_Overlay
...
# Merge dfs
Mega_df <- cbind(Z_means_P,Z_means_I,y_df)
# Add time steps
ts <- seq(nrow(Mega_df))
ts <- data.frame(ts)
# Final df
...
Post_By_Data_Plot <- ggplot(Mega_df,aes(x=ts))+
  ...
  geom_line(aes(y=I,colour="cornflowerblue"))+ # Data I
  geom_line(aes(y=post_means_P,colour="lightgreen"),size=1.5)+ # Post. P
  geom_line(aes(y=post_means_I,colour="lightskyblue1"),size=1.5)+ # Post. I
...
Post_By_Data_Plot <- ggplot(Mega_df,aes(x=ts))+
  ...
  geom_line(aes(y=post_means_P,colour="lightgreen"),size=1.5)+ # Post. P
  geom_line(aes(y=post_means_I,colour="lightskyblue1"),size=1.5)+ # Post. I
  scale_x_continuous(breaks = seq(0, 50, by = 10)) + # Cleaning up the axes
...
## Basic model checks ##
Fit_Antia_Summ <- print(Fit_Antia, pars=c("r", "k", "p", "o", "sigma"),
                        probs=c(0.1, 0.5, 0.9), digits = 3)
parms <- c("r", "k", "p", "o")
