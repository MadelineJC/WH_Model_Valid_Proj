...
July1_Plot <- ggplot(July1_DF,aes(x=time))+
  geom_line(aes(y=X1, color="cornflowerblue"), size = 1)+ # Parasite
  geom_line(aes(y=X2,color="springgreen4"), size = 1)+ # Host
  theme_minimal()+
  xlab(" ")+
  ylab(" ")+
  ggtitle("")+
...
write_csv(July1_Data,here('Desktop/GitHub/FcnalRespProj_2/Data/NewData.csv'))
July1_Plot <- ggplot(July1_Data,aes(x=t))+
  geom_line(aes(y=P, color="cornflowerblue"), size = 1)+ 
  geom_line(aes(y=H,color="springgreen4"), size = 1)+
  theme_minimal()+
  xlab(" ")+
  ylab(" ")+
  scale_color_manual(name="Legend",
